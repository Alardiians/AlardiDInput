...
6. **Multipliers:** weapon `HeadMult` is applied on head hits when the enemy has a head zone (enemy `HeadMult` > 1.0 just flags the zone — never multiply the two). Precision weapons on weak points use max(enemy `WeakPointMult`, weapon `HeadMult`). Weapon `ShieldMult` applies vs. Bulwark shields and Harmony Walls; Gilded plates use the damage-type table in `04_Core_Gameplay_Loop.md` §10.3.
7. **Re-generating:** these files were produced from the GDD by script; if you change a value, change it here and note it in `BALANCE_LOG.md`. Prose documents may lag behind the CSVs during balancing — the CSV wins.
...
* Brass to max all weapon upgrades = **262** (~341 available; see `08_Progression_and_Economy.md` §1).
